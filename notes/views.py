...
class ShareNoteWithDetail(APIView):
    ...
    def post(self, request, note_id):
        note = get_note_or_404(note_id)
        if note.user != request.user:
            return Response(NoteMessages.FORBIDDEN_SHARE, status=status.HTTP_403_FORBIDDEN)
        ...
        if email_serializer.is_valid():
            ...
            shared = SharedNote.objects.filter(note=note, recipient_user=user).exists()
            if not shared:
                shared_note.save()
